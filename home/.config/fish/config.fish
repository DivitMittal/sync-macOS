...
# fish_add_path $brew_prefix/opt/ed/bin;
...
if status is-interactive
  ...
  # set -x LESSHISTFILE '-'
  set -x LESSOPEN "|$(which lesspipe.sh) %s"
  ...
  fish_add_path /Application/Floorp.app/Contents/MacOS # Floorp browser
  ...
  alias ff 'fastfetch --logo-type iterm --logo $HOME/Sync-macOS/assets/a-12.png \
    --pipe false --title-color-user magenta --title-color-at blue --title-color-host red \
    --structure Title:OS:Kernel:Uptime:Display:Terminal:CPU:CPUUsage:GPU:Memory:Swap:LocalIP --gpu-temp true --cpu-temp true --cpu-format "{1} @ {#4;35}{8}{#}" --gpu-format "{2} @ {#4;35}{4}{#}"'
  ...
  alias brew-ultimate 'brew update; and brew upgrade; and brew autoremove; and brew cleanup -s --prune=0; and rm -rf (brew --cache)'
